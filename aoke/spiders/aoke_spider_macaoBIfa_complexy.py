...
class SoccerSpider(scrapy.Spider):
    ...
    def match_macao_parse(self, response):
        ...
        # 遍历澳门，查看其是否变盘，若变过盘才能继续
        has_change_handicap = False
        ...
        for tr in response.xpath('//tbody')[0].xpath('tr[@class=""]'):
            ...
            if pre_handicap != '':
                if current_handicap != pre_handicap:
                    has_change_handicap = True
            pre_handicap = current_handicap

        # 如果没有变盘，就直接返回，不进行判断
        if not has_change_handicap:
            return False
    ...
    def match_bifa_parse(self, response):
        ...
        if support_direction == response.meta['beginning_price_deny']:
            support_direction = 0
