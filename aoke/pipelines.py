        ...
        try:
            with connection.cursor() as cursor:
                ...
                build_table = (
                    ...
                    "macaoBifa_support_direction INT(4) NOT NULL,"
                ...
                update_sql = (
                    "UPDATE "+tableName+" SET host_goal="'%s'", guest_goal="'%s'", is_end= %d, macaoBifa_support_direction=%d, algorithm_score=%f "
                ...
                try:
                    if table_row_len < 1:
                        print('insert数据库')
                        cursor.execute(insert_sql % (item['match_id'], item['host'], item['guest'], item['league_name'], item['start_time'],item['host_goal'],item['guest_goal'], item['is_end'], item['macao_handicap'], item['macaoBifa_support_direction'], item['algorithm_score']))
                    else:
                        print('update数据库')
                        cursor.execute(update_sql %
                       (item['host_goal'], item['guest_goal'], item['is_end'], item['macaoBifa_support_direction'], item['algorithm_score'])
                        )
...
class AokeTodayPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        try:
            with connection.cursor() as cursor:
                # 设置当前表名
                tableName = 'aoke_macao_bifa_'+nowatime
                # 建表
                build_table = (
                    "CREATE TABLE IF NOT EXISTS "' %s '""
                    "(match_id VARCHAR(16) NOT NULL PRIMARY KEY,"
